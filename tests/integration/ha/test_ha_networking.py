...
async def test_full_network_cut_with_ip_change_node_with_primary_shard(
    ...
    # Shard recovery onto the rejoined unit happens asynchronously (it is not
    # reflected by Juju's idle/active status), so poll instead of asserting on a
    # single snapshot to avoid a race with OpenSearch's own rebalancing.
    assert await wait_until_unit_hosts_shard(
        ops_test, leader_unit_ip, ContinuousWrites.INDEX_NAME, first_unit_with_primary_shard
    ), (
        f"Unit {first_unit_with_primary_shard} never hosted a replica shard of "
        f"'{ContinuousWrites.INDEX_NAME}' after rejoining the cluster."
    )
